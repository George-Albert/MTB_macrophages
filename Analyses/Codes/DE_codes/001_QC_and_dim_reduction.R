...
###
### 1. Set labeling of samples
###

{
    cols=read.table("inputs/raw/metadata.txt",header=TRUE)
    cols$Setup=paste0(cols$Condition,"_",cols$TimePoint)
    setups=unique(cols$Setup)
    
    reads=read.table("inputs/raw/reads.txt",header=TRUE,check.names=FALSE)
    rownames(reads)=reads[,1]
    reads=reads[,2:ncol(reads)]
    
    
    ### Hay algunas samples con el SampleID duplicado en la tabla de metadatos, en las
    ### colnames de reads se diferencian con un .1. Asumo que esto no es problema para saber cuál de las columnas se corresponde a un duplicado porque están en el mismo orden en data (columnas) y metadata (rows).
    ...
    ### Así que añado a mano los .1 también en los metadatos, en una nueva columna (que almacenará los sampleIDs corregidos.
    cols$Sample=cols$SampleID
    cols$Sample[set_differ]=paste0(cols$SampleID[set_differ],".1")
    
    ### This is to see that all these dups appeared two times onl
    length(unique(cols$Sample))
    ## 883
    nrow(cols)
    ## 883
    
    ## This is provisional, rownames will be redeclared later
    rownames(cols)=cols$Sample
    
    length(which(rownames(cols)!=colnames(reads)))
    length(which(cols$Sample!=colnames(reads)))
    
    ### AHORA CORRIJO LOS .1 Y LOS PONGO EN LAS MUESTRAS CON DUPLICATED==1, QUE TERMINARAN POR NOI USARSE
...
###
### 2. Check sample depths (specially in Duplicated samples) & Check for flowcell merging (i.e. are the repeated samples new sequencing instances, or the merging between the old and the new one?)
###

{
    ...
    pl=ggplot()+geom_jitter(aes(x=Flowcell,y=Original_depth,color=as.character(Duplicated)),data=tabla)+coord_flip()
    
    ### From visual inspection, these are samples that will be good to track bc/ of different reasons (To that I define the variable QC_label
    
    ## Deeper outliers than their Flowcell averages
    set_1=which((cols$Flowcell %in% c("171211_K00243_0104_BHLVWHBBXX","171124_K00243_0098_AHLNGVBBXX") & cols$Original_depth>3E7) | cols$Original_depth>3.5E7)
    ...
    pl=ggplot(cols)+
    geom_jitter(aes(x=Flowcell,y=Original_depth,color=as.character(Duplicated)),position = position_jitter(seed = 1))+
    geom_text_repel(aes(x=Flowcell,y=Original_depth,label=QC_labels),position = position_jitter(seed = 1))+
    coord_flip()
    
    system("mkdir -p outputs/1_QC")
    pdf("outputs/1_QC/depths_per_FC.pdf",width=12,height=6)
    print(pl)
    dev.off()
    
     ## Two more sanity checks:
     ## Check that all samples that are not unique form pairs (i.e. no sample was sequenced more than twice=)
     
     # Remember that, for each sample ID that appear more than once, Duplicated=1 is the first instance and =2 all the others).
     length(which(cols$Duplicated==1))
     length(which(cols$Duplicated==2))

     ## So, it is ok.
     
     ## Another sanity check: in all the samples that are repeated, the repetitions are resequenced instances, and one is not a merging of two sequencing instances instead.
    ...
    # All samples have some entries that are higher than the resequenced pair, and some that are lower: none is a merging.
       
       
    ### Conclusion: There was a funky flowcell, and a couple of other bad samples that, once repeated, yielded good samples with two exceptions (C50_0_72 & C60_TB_72). A third sample was also bad, but was not resequenced (C12_0_2h). As for the rest, some of them could be eventually removed if we were to get picky regarding library depths distributions, but I don´t think that is necesary.
...

{
    ...
    pl_Depths_distribution=ggplot(tab)+geom_jitter(aes(x=x_jit,y=Original_depth,color=as.factor(Duplicated)))+geom_violin(aes(x=x,y=Original_depth),fill=NA)+
    ...
    geom_hline(yintercept=35000000,color="black")+theme_classic()
    
    pdf("outputs/1_QC/global_depth_distribution.pdf",width=6,height=6)
...

{
    ...
    ## This is to collapse the Setup columns popolated with 0s and 1s into a binary string of length(setusps) bits
    ...
    ### 6
    ### These "pity" guys are the individuals whose samples got sequenced in more than one flowcell. This is a pity, bc if Individuals were perfectly nested within flowcells that would have simplified a lot the model designs).
    pity=tab[(which(tab$fcs>1)),]
    pity_cols=cols[which(cols$IndividualID %in% pity$ind),]
    
    ### Exploring this pity_cols table, I identify visually the 14 samples that one should remove (that's the minimum amount) to recover a perfect nesting of individuals within flowcells. Alternatively, we could use the variable intrcpt to model fixed-effect intercepts (it's almost always the individual, except for these 6 pity guys for which an individual is splited in 2 flowcells (we would fit 2 intercepts for each of them).
    
    pity_samples=c("10_0_2h","10_TB_2h","C053_0_72h","C13_0_48h","C13_0_72h","C16_0_72h","C23_0_18h","C23_0_2h","C23_TB_18h","C23_TB_2h","C9_0_18h","C9_0_2h","C9_TB_18h","C9_TB_2h")
    
    cols$Ind_FC_nesting_criterium=1
    cols$Ind_FC_nesting_criterium[which(rownames(cols) %in% pity_samples)]=0
    cols$Notes[which(rownames(cols) %in% pity_samples)]=paste0(cols$Notes[which(rownames(cols) %in% pity_samples)],"_Breaks_Ind_in_FC_nesting")
    
    cols_whole$Ind_FC_nesting_criterium=1
    cols_whole$Ind_FC_nesting_criterium[which(rownames(cols_whole) %in% pity_samples)]=0
    cols_whole$Notes[which(rownames(cols_whole) %in% pity_samples)]=paste0(cols_whole$Notes[which(rownames(cols_whole) %in% pity_samples)],"_Breaks_Ind_in_FC_nesting")
    
    
    
    ## From C23 and C9 I label as pity the early ones because I lack late samples for more individuals...
    ...
    write.table(tab,"outputs/1_QC/distribution_of_flowcells_across_conditions_upon_filtering_one.txt")
    write.table(tab_ind,"outputs/1_QC/distribution_of_individuals_across_conditions_upon_filtering_one.txt")
    ...
    tabm=melt(tabm,ID="ind")
       ...
       pl_bars=ggplot(tab_ind)+geom_point(aes(x=ind,y=as.character(fcs)))+
       theme_classic()+
       theme(
       ...
       pdf("outputs/1_QC/setup_distribution_across_flowcells.pdf",width=10,height=3)
         print(pl_tiles)
         dev.off()
         
         pdf("outputs/1_QC/setup_distribution_across_individuals.pdf",width=8,height=12)
           print(pl_tot)
           dev.off()
...
###
### 5. Second filtering: pity samples out: now individual is nested in flowcell
###
...
###
### 6. Low expression filtering first on those 2 filtering (depth outliers and FC_ind_nesting breakers).
###

{
    ...
    ### Option A: I select genes that are expressed at a threshold of log2(cpm)=1 in at least one of the conditions of interest (the first control and all the infected conditions) (I got 12989 genes)
    ### Option B: for considering a gene, I'll ask it to have at least a median(log2(cpm))>2 in any of the ten combinations condition-time. At that threshold, two things happen:
    ### 1-The # of genes that pass is still nice (11548) and doesn't drop much from the maybe more usual threshold of log(cpm)=1 (12989).
    ### One can disregard the dychotomy of whether using all the 10 setups to do this filtering, only the relevant (or even fancier, model all genes, remove the effects of time in the control as we'll do later, and do the filtering on the relevant conditions, once the time has been regressed out, model based). This is because, of the 262 genes that pass the threshold by considering all conditions which would have failed if considering only the relevant ones, only 13 would have failed also, considering the relevant ones, if using a median log(cpm)=1 as a threshold (so most of them are still fairly expressed in at least one of the interesting conditions too.
...
### Creating new columns (Batch, Individual, Treatment, Time and, Sample) with the smae info of what it is in there already, but clean, for common use later.
###
...

{
    ...
    get_PCA=function(setups,name,type="infection"){
        ...
        system("mkdir -p outputs/1_QC/PCAs/clean/plots")
        system("mkdir -p outputs/1_QC/PCAs/clean/summaries")
        system("mkdir -p outputs/1_QC/PCAs/clean/data")
        
        system("mkdir -p outputs/1_QC/PCAs/dirty/plots")
        system("mkdir -p outputs/1_QC/PCAs/dirty/summaries")
        system("mkdir -p outputs/1_QC/PCAs/dirty/data")
        ...
        )+xlab(paste0("PC1 (",round(sum_pca$PC1[2],3)*100,"% variance)"))+ylab(paste0("PC2 (",round(sum_pca$PC2[2],3)*100,"% variance)"))
        
        pdf(paste0("outputs/1_QC/PCAs/dirty/plots/",name,".pdf"),width=6,height=4)
        print(pl)
        dev.off()
        
        sink(paste0("outputs/1_QC/PCAs/dirty/summaries/",name,".txt"))
        print(sum_pca)
        sink()
        
        write.table(datos,paste0("outputs/1_QC/PCAs/dirty/data/",name,".txt"))
        ...
        pdf(paste0("outputs/1_QC/PCAs/clean/plots/",name,".pdf"),width=6,height=4)
        print(pl)
        dev.off()
        
        sink(paste0("outputs/1_QC/PCAs/clean/summaries/",name,".txt"))
        print(sum_pca)
        sink()
        
        write.table(datos,paste0("outputs/1_QC/PCAs/clean/data/",name,".txt"))
...

{
    ...
    label_PCA=function(data,ranges,name){
        system("mkdir -p outputs/1_QC/PCAs/clean/plots_labelled")
        system("mkdir -p outputs/1_QC/PCAs/clean/data_labelled")
        ...
        pdf(paste0("outputs/1_QC/PCAs/clean/plots_labelled/",name,".pdf"),width=8,height=6)
        print(pl)
        dev.off()
        
        write.table(datos_clean,paste0("outputs/1_QC/PCAs/clean/data_labelled/",name,".txt"))
...
####
#### 11. Repeat PCAs after correcting misslabels, and label outliers a bit less aggressively (or I would be thrashing too many 20h samples, probably.(Omit all-times-together,non-interesting here)
####

{
    ...
    get_PCA_bis=function(setups,name,type="infection"){
        ...
        system("mkdir -p outputs/1_QC/PCAs/clean_bis/plots")
        system("mkdir -p outputs/1_QC/PCAs/clean_bis/summaries")
        system("mkdir -p outputs/1_QC/PCAs/clean_bis/data")
        
        system("mkdir -p outputs/1_QC/PCAs/dirty_bis/plots")
        system("mkdir -p outputs/1_QC/PCAs/dirty_bis/summaries")
        
        system("mkdir -p outputs/1_QC/PCAs/dirty_bis/data")
        ...
        pdf(paste0("outputs/1_QC/PCAs/dirty_bis/plots/",name,".pdf"),width=6,height=4)
        print(pl)
        dev.off()
        
        sink(paste0("outputs/1_QC/PCAs/dirty_bis/summaries/",name,".txt"))
        print(sum_pca)
        sink()
        
        write.table(datos,paste0("outputs/1_QC/PCAs/dirty_bis/data/",name,".txt"))
        ...
        pdf(paste0("outputs/1_QC/PCAs/clean_bis/plots/",name,".pdf"),width=6,height=4)
        print(pl)
        dev.off()
        
        sink(paste0("outputs/1_QC/PCAs/clean_bis/summaries/",name,".txt"))
        print(sum_pca)
        sink()
        
        
        datos_clean=datos
        write.table(datos_clean,paste0("outputs/1_QC/PCAs/clean_bis/data/",name,".txt"))
    ...
    label_PCA_bis=function(data,ranges,name){
        system("mkdir -p outputs/1_QC/PCAs/clean_bis/plots_labelled")
        system("mkdir -p outputs/1_QC/PCAs/clean_bis/data_labelled")
        ...
        pdf(paste0("outputs/1_QC/PCAs/clean_bis/plots_labelled/",name,".pdf"),width=8,height=6)
        print(pl)
        dev.off()
        
        write.table(datos_clean,paste0("outputs/1_QC/PCAs/clean_bis/data_labelled/",name,".txt"))
        return(list(datos_dirty=datos_dirty,datos_clean=datos_clean,sum_pca=sum_pca))
    }
    {
        ranges_df=data.frame(
    ...
    ## I will add to samples_to_remove I_C14_NI_72h and I_C13_TB_20h: these belong to the only pairs of TB/NI samples that, once the PCA outliers are removed, remains clumping together (I_C14_NI_72h), or still go with the other cluster in the tsne (I_C13_TB_20h).
    
    samples_to_remove=c(samples_to_remove,"I_C14_NI_72h","I_C13_TB_18h")
    
    ## samples_to_remove=c("I_C59_NI_48h","I_4_NI_48h","I_4_NI_72h","I_C59_NI_72h","I_C4_TB_18h","I_C5_TB_18h","I_C5_TB_48h","I_C60_TB_20h","I_C60_TB_48h","I_3_TB_48h","I_3_TB_72h","I_42_TB_48h","I_42_TB_72h","I_C32_TB_48h","I_C32_TB_72h","I_C5_TB_72h","I_C14_NI_72h")
...
###
### 12. See that removing these last samples do not reduce the time-wise signal (before r´setting the geneset upon final low expression thresholding)
###

{
    ...
    ###
    ### Get a signal estimate of contrasts associated with this current setup (i.e. pity samples out, outliers out)
    ###
...
### 5. Samples that are outliers in the PCAs: removing these time until 48 h separates in PC1 in adjacent time points wout lowering the signal, as tested in the part of the coded commented below

## "I_C59_NI_48h" "I_4_NI_48h"   "I_4_NI_72h"   "I_C59_NI_72h" "I_C4_TB_18h"  "I_C5_TB_18h"  "I_C5_TB_48h"  "I_C60_TB_20h" "I_C60_TB_48h" "I_3_TB_48h"   "I_3_TB_72h"   "I_42_TB_48h"  "I_42_TB_72h"  "I_C32_TB_48h" "I_C32_TB_72h" "I_C5_TB_72h"

## One more sample that looks like a contamination after doing tsne & PCA, once all other samples have been corrected/removed:

## 6. I_C14_NI_72h I_C13_TB_18h

## On top of that, nothing seems to indicate a technical or biological difference between time=20h and time=18h.

}

###
### 14. Build final tables to use: the reads are filtered for low expression using the dataset that verifies Depth, nesting and PCA; and the metadata only has the important, groomed columns. Also, merge timepoints 18 ahd 20.
###
...

{
    ...
    library(biomaRt)
    ensembl <- useMart(host="jul2018.archive.ensembl.org", biomart="ENSEMBL_MART_ENSEMBL", dataset="hsapiens_gene_ensembl")
    hugos <- getBM(attributes=c('hgnc_symbol', 'ensembl_gene_id','gene_biotype'),filters = 'ensembl_gene_id', values = rownames(reads), mart = ensembl)

    hugos=hugos[which(hugos$gene_biotype=="protein_coding"),]

    length(which(hugos$hgnc_symbol==""))
    ## 59 non-available gene names, I give them the ensembl ID also in the gene name column of the dictionary
...
###
### 17. (homogenize genotype samples IDs in the DNA data with Individuals´notation here), and annotate
### in the RNA metadata those samples having vs lacking genotype data.
###

{

    system("mkdir -p inputs/processed/genotype_matrix_building_files")
    sample_IDs=read.table("inputs/raw/genotype_IDs.txt",header=TRUE)
    sample_IDs_down=read.table("inputs/raw/genotype_IDs_tuned_preliminar.txt",header=TRUE)
    sample_IDs_down$Individual=paste0("I_",sample_IDs_down$Individual)

    metadata=whole_groomed

    ### Aqui ya veo que hau individual IDs en distinto formato en el RNA que en el DNA
    ...
    ## Ya que las columnas de ordinal, individual y type van a ser raw, dejo vacio el que estaba vacio.
    ...
    library(openxlsx)
    write.xlsx(sample_IDs_down,"inputs/raw/genotype_IDs_preliminar2.xlsx")
    ...
    ## Likely, one of thw two DNA samples named I_C29 correspond to I_29 in the RNAseq: in there, we do have a guy named I_29, and another one named I_C29.
...

{
    ...
    system("mkdir -p inputs/processed/whole")
    system("mkdir -p inputs/processed/ready_for_DE")
    system("mkdir -p inputs/processed/ready_for_EQTL")

    write.table(reads,"inputs/processed/ready_for_DE/reads.txt")
    write.table(cols,"inputs/processed/ready_for_DE/metadata.txt")
    write.table(dictionary,"inputs/processed/ready_for_DE/dictionary.txt")

    ## In the whole tables I do not use HUGOs (didn´t check how many issues are in there, but I don´t quite need that)
    write.table(reads_whole,"inputs/processed/whole/reads_whole.txt")
    write.table(whole_groomed,"inputs/processed/whole/metadata_whole.txt")
    ...
    ## In the files ready for EQTL mappin I will use the same geneset to simplify the situation.
    ...
    write.table(reads_EQTL,"inputs/processed/ready_for_EQTL/reads.txt")
    write.table(cols_EQTL,"inputs/processed/ready_for_EQTL/metadata.txt")
    write.table(dictionary,"inputs/processed/ready_for_EQTL/dictionary.txt")
...
get_PCA_tris=function(setups,name,type="infection",shift1=1,shift2=1){
    ...
    system("mkdir -p outputs/1_QC/PCAs/clean_tris/plots")
    system("mkdir -p outputs/1_QC/PCAs/clean_tris/summaries")
    system("mkdir -p outputs/1_QC/PCAs/clean_tris/data")
    
    system("mkdir -p outputs/1_QC/PCAs/dirty_tris/plots")
    system("mkdir -p outputs/1_QC/PCAs/dirty_tris/summaries")
    
    system("mkdir -p outputs/1_QC/PCAs/dirty_tris/data")
    ...
    pdf(paste0("outputs/1_QC/PCAs/dirty_tris/plots/",name,".pdf"),width=6,height=4)
    print(pl)
    dev.off()
    
    sink(paste0("outputs/1_QC/PCAs/dirty_tris/summaries/",name,".txt"))
    print(sum_pca)
    sink()
    
    write.table(datos,paste0("outputs/1_QC/PCAs/dirty_tris/data/",name,".txt"))
    ...
    pdf(paste0("outputs/1_QC/PCAs/clean_tris/plots/",name,".pdf"),width=6,height=4)
    print(pl)
    dev.off()
    
    sink(paste0("outputs/1_QC/PCAs/clean_tris/summaries/",name,".txt"))
    print(sum_pca)
    sink()
    
    write.table(datos,paste0("outputs/1_QC/PCAs/clean_tris/data/",name,".txt"))
...

{
    ...
    system("mkdir -p outputs/1_QC/tsnes/")
    pdf(paste0("outputs/1_QC/tsnes/tsne.pdf"),width=6,height=6)
    print(tsne_clean[[1]])
    dev.off()
    
    write.table(tsne_clean[[2]],"outputs/1_QC/tsnes/tsne_data.txt")
}
