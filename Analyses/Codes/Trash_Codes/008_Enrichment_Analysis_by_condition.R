...
for (i in vec_to_loop) {
  ...
  file_name <- file.path(enrichment_dir,paste0(names_to_save[i], ".txt"))
  
  # Save the character vector to a file
  write.table(list_DE_infected_overtime[[i]], file_name,col.names = F,row.names = F,
              quote = F)
}
...
vector_of_names <- names(list_DE_infected_overtime)

### Zero Configuration: To get a view of the stats
config_dir <- "zero_Configuration"
...
vec_to_loop <- seq_along(list_DE_infected_overtime)
# vec_to_loop <- c(4,7,8,9,10,12,14,15,16)
vec_to_loop <- c(4)
...
for (i in vec_to_loop) {
  ...
  query_genes <- unlist(list_DE_infected_overtime[i])
  
  print(paste(" I´m working with the condition of:",names(list_DE_infected_overtime[i])))
