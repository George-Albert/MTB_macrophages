...
filter_list <- function(x, OR_threshold, fdr_threshold) {
  x <- x[x$OR > OR_threshold & x$fdr < fdr_threshold, ]  # filter by rows
  if (nrow(x) > 0) return(x) else return(NULL)  # Return NULL if df is empty
}
...
# 
# lista_filtered <- list(Early=Early,Mid=Mid,Late=Late)
...
order_clusters <- c(7,3,1,8,9)
row_order_index <- row_order_index[order(factor(row_order_index$Cluster,levels =order_clusters )),]
rownames(row_order_index) <- row_order_index$description
rownames(union_of_df) <- union_of_df$description
union_of_df <- union_of_df[rownames(row_order_index),]
length(which(rownames(row_order_index)!=rownames(union_of_df)))
...
# Early <- rbind(list_enrichment_to_clean$`k=7`,list_enrichment_to_clean$`k=3`)
# Mid <- rbind(list_enrichment_to_clean$`k=1`,list_enrichment_to_clean$`k=8`)
# Late <- list_enrichment_to_clean$`k=9`
# 
# list_enrichment_to_clean <- list(Early=Early,Mid=Mid,Late=Late)
# row_order_index$des_cluster <- paste0(row_order_index$description,"_", row_order_index$Cluster)

# filtered_list <- lapply(list_enrichment_to_clean, function(df) {
#   # df$des_cluster <- paste0(df$description,"_", df$Cluster)
#   # df[df$des_cluster %in% row_order_index$des_cluster,]
#   df[df$description %in% row_order_index$description,]
#   # return(df)
# })
...
df <- df[rownames(row_order_index),]
...
hclust_matrix <- apply(hclust_matrix, MARGIN=c(1, 2), FUN =function(x) log2(x + 1))

# Min-Max Scaling Normalization between 0 y 1
normalize_min_max <- function(x) {
  return((x - min(x)) / (max(x) - min(x)))
}

hclust_matrix <- apply(hclust_matrix, 2, normalize_min_max)  # Normalizar por columna
...
columns_name <- c("Mid1","Early1","Early2","Mid2","Late")
colnames(hclust_matrix) <- columns_name
level_to_use <- c("Early1","Early2","Mid1","Mid2","Late")
...
write.table(hclust_df,file = file.path(input_folder,paste0("Cluster_by_stage_normalized.txt")))

dir.create(file.path(output_dir,"001_Figures","005_Enrichment"),recursive=T,showWarnings =F)
filename <- file.path(output_dir,"001_Figures","005_Enrichment",paste0("Heatmap_GO_by_stage.pdf"))
filename <- file.path(output_dir,"001_Figures","005_Enrichment",paste0("Heatmap_GO_by_stage_normalized.pdf"))
