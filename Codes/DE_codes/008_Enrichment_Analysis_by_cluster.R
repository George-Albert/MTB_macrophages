...
##################################
{
  script_id  <- "008_Enrichment_Analysis_by_cluster"
  ...
  # Set the directory for boolean matrices and metadata
  boolean_matrices_dir <- file.path(input_dir,"004_GO_data","003_Boolean_matrices")
  metadata_dir         <- file.path(input_dir,"004_GO_data","004_Metadata_matrix")
  
  GO_gene_term_matrix_bp <- readRDS(file.path(boolean_matrices_dir,"boolean_matrix_bp.RDS"))
  BP_metadata_matrix     <- readRDS(file.path(metadata_dir,"BP_metadata_matrix.RDS"))
  GO_gene_term_matrix_cc <- readRDS(file.path(boolean_matrices_dir,"boolean_matrix_cc.RDS"))
  CC_metadata_matrix     <- readRDS(file.path(metadata_dir,"CC_metadata_matrix.RDS"))
  GO_gene_term_matrix_mf <- readRDS(file.path(boolean_matrices_dir,"boolean_matrix_mf.RDS"))
  MF_metadata_matrix     <- readRDS(file.path(metadata_dir,"MF_metadata_matrix.RDS"))
  ...
  ###and their genes genes are 19920 1990
...
##############
{
  ...
  cluster_out1 <- cluster_out[,c(1:4,6)] # Selecting k=16 clusters column
  ...
  df_counts <- data.frame(
    Cluster.Name = names(list_per_clusters),  
    Gene.Count = sapply(list_per_clusters, length) 
  )
  ...
  ### k= 14 Persistent Up            
  ### k= 7 Early Down, k= 1 Mid Down,k= 4 Late Down
  ### k= 3 Early Up, k= 8 Mid Up,k= 9 Late Up
  
  ### Now we are going to redefine the groups attending to the order of the clusters
  # 1,6,10  Graduall Down now
  # 3,5,7   Graduall Up now
  # 8       Persistent Down now
  # 13      Persistent Up now
  ### k= 6 Early Down, k= 1 Mid Down,k= 10 Late Down
  ### k= 3 Early Up,   k= 7 Mid Up,  k= 5 Late Up
  
  ##Now 
  ### k= 3,11,14 Graduall Down
  
  # Define how to group the data frames
  group_indices <- list(6,1,10,3,7,5,8,13)  # Each sublist represents a group of indices
  group_names <- c("Early.Down", "Mid.Down","Late.Down",
                   "Early.Up"  , "Mid.Up"  ,"Late.Up",
                   "Persistent.Down", "Persistent.UP")
  ...
  length(background)
  # 10753
  writeLines(background,file.path(enrichment_dir,"background_list.txt"))
...
##############
{
  ...
  # config_dir <- "zero_Configuration_by_stage"
  # 
  # {
  #   min_level <- 3
  #   max_level <- 8
  ...
  ## 4th Configuration more permissive values
