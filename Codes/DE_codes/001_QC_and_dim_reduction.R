...

{
  ...
  length(unique(cols$Flowcell))     # 10 flowcells
  length(unique(cols$Condition))    # 2 conditions
  length(unique(cols$TimePoint))   # 5 time points at the moment
  length(unique(cols$IndividualID)) # 102 individuals
  length(unique(cols$SampleID))     # 785 samples (some duplicated))
  
  duplicated_samples=cols$SampleID[which(duplicated(cols$SampleID))]
  
  # Create Setup column with the union of Condition and Time Point
...
##############
{
  ...
  # colors <- c("0" = "#0072B2",   # azul intenso
  #             "1" = "#E69F00",   # naranja
  #             "2" = "#D55E00")
  
  colors <- c("#F8766D", "#00BA38","#619CFF")
  
  pl <- ggplot(tabla, aes(x = Original_depth, y = Flowcell)) +
    # Main points
    geom_jitter(aes(color = as.character(Duplicated)),
                width = 0.2, height = 0.2, size = 3, alpha = 0.9) +
    # Labels just for duplicated samples
    geom_text_repel(
      aes(label = Sample),
      data = subset(tabla, Duplicated %in% c(1, 2)),
      size = 3.2, 
      max.overlaps = 60,
      box.padding = 0.5,
    ...
    scale_color_manual(values = colors, name = "Duplicated") +
    
    # etiquetas y título
    labs(
      title = "Sequencing Depth and Flowcell Distribution",
      subtitle = "Samples colored by duplication status",
      x = "Original Depth",
      y = "Flowcell"
    ) +
    
    # tema tipo publicación
  ...
  # Save data to plot
  write.table(tabla, file.path(data_to_plot_dir, "001_Depths_per_FC.txt"),
  ...
  cols$QC_labels[samples_to_label]=cols$Sample[samples_to_label]
  ...
  pl1 <- ggplot(cols) +
    geom_jitter(
      aes(x = Flowcell, y = Original_depth, color = as.character(Duplicated)),
      position = position_jitter(seed = 1, width = 0.2),
      alpha = 0.8,
      size = 3
    ) +
    geom_text_repel(
      aes(x = Flowcell, y = Original_depth, label = QC_labels),
      position = position_jitter(seed = 1, width = 0.2),
      size = 3.5,
      max.overlaps = 40,
      show.legend = FALSE
    ) +
    geom_hline(yintercept = 1e7, color = "grey50", linetype = "dashed", linewidth = 0.6) +
    coord_flip() +
    scale_color_manual(values = colors) +
    labs(
      title = "Sequencing Depth per Flowcell",
      subtitle = "Samples colored by duplication status",
      x = "Flowcell",
      y = "Original Depth",
      color = "Duplicated"
  ...
  # Save data to plot
  write.table(cols, file.path(data_to_plot_dir, "001_Depths_per_FC_wline.txt"),
              sep = "\t", quote = FALSE, row.names = TRUE)
  
  dir.create(file.path(output_dir,"001_Figures","001_QC"),recursive = T,showWarnings = F)
  ...
  pdf(file.path(QC_dir,"001_Depths_per_FC_wline.pdf"),width=12,height=6)
  print(pl1)
  dev.off()
  ...
  test=data.frame(Unos=colnames(unos),Doses=colnames(doses))
  
  # Check that the pairing is correct
  test$Unos=test$Unos[order(test$Unos)]
  test$Doses=test$Doses[order(test$Doses)]
  
  test$check=paste0(test[,1],".1")
  length(which(test[,2]!=test[,3])) ### check this columns order later if needed
  ...
  ##                distributions, but I don´t think that is necessary.               ##
  #####################################################################################
  
  ### For now, let's take the 1E7 QC cutoff here.
  cols$Depth_criterium=0
  cols$Depth_criterium[which(cols$Original_depth>1E7)]=1
  length(which(cols$Depth_criterium==1)) # 782
  
  ###############################################################################################
  ## First sample subsetting (according to Depth_criterium, declare _safe objects first).  ##
  ###############################################################################################
  cols_whole=cols
  reads_whole=reads
  ...
  pl_Depths_distribution=ggplot(tab)+
    ...
    geom_hline(yintercept=5E6,color="grey90")+
    geom_hline(yintercept=1E7,color="grey80")+
    geom_hline(yintercept=15E6,color="grey60")+
    geom_hline(yintercept=35E6,color="grey40")+
    geom_hline(yintercept=4E7,color="black")+
    labs(color="Duplicated")+
    scale_colour_manual(values = colors[c(1,3)])+
    theme_classic()+
    theme(
      ...
      plot.title = element_text(size = 16)  # graph title size
    )
  pl_Depths_distribution
  
  
  pdf(file.path(QC_dir,"002_Global_depth_distribution.pdf"),width=7.5,height=9)
  print(pl_Depths_distribution)
  dev.off()
  
  # Save data to plot
  write.table(tab, file.path(data_to_plot_dir, "002_Global_depth_distribution.txt"),
              sep = "\t", quote = FALSE, row.names = TRUE)
}
...
{
  #########################################################################################
  ##     Basic exploration on how are the individuals and flowcells distributed          ##
  ...
  ## binary string of length(setups) bits
  ...
  ###############################################################################
  ##  These ´pity´ guys are the individuals whose samples got sequenced in  ##
  ...
  tabm <- tabm %>%
    mutate(highlight = case_when(
      grepl("18h", variable, ignore.case = TRUE) ~ "18h",
      grepl("20h", variable, ignore.case = TRUE) ~ "20h",
      TRUE ~ "Other"
    ))
  
  vmin <- min(tabm$value, na.rm = TRUE)
  vmax <- max(tabm$value, na.rm = TRUE)
  
  pl_tiles <- ggplot(tabm, aes(variable, fc)) +
    # Base layer: all the samples except 18h and 20h
    geom_tile(data = subset(tabm, !highlight %in% c("18h", "20h")),
              aes(fill = value),
              color = "black", linewidth = 0.8) +
    scale_fill_gradient(
      low = "white", high = "#B63679FF",limits = c(vmin, vmax),
      breaks = seq(vmin, vmax, length.out = 4), name = "2h, 48h, 72h") + 
    ggnewscale::new_scale_fill() +
    #Layer for highlighted samples: 18h and 20h
    geom_tile(
      data = subset(tabm, highlight %in% c("18h", "20h")),
      aes(fill = value),
      color = "black", linewidth = 1) +
    scale_fill_gradient(
      low = "white", high ="#21908CFF",limits = c(vmin, vmax),
      breaks = seq(vmin, vmax, length.out = 4), name = "18h and 20h") +
    # Overlay text labels
    ...
    labs(
      title = "Sample Yield Across Flowcells",
      x = "Samples",
      y = "Flowcells"
    )
  ...
  # Save data to plot
  write.table(tabm, file.path(data_to_plot_dir, "003_Flowcell_distribution.txt"),
              sep = "\t", quote = FALSE, row.names = TRUE)
  ...
  pl_tiles2 <- ggplot(tabm, aes(x = variable, y = ind)) +
    geom_tile(
      data = subset(tabm, !highlight %in% c("18h", "20h")),
      aes(fill = value),
    ...
    scale_fill_gradientn(
      colours = c("white", "#E58EB0"),limits = c(vmin, vmax),
      breaks = seq(vmin, vmax, length.out = 2), name = "2h, 48h, 72h") +
    ggnewscale::new_scale_fill() +  
    geom_tile(
      data = subset(tabm, highlight %in% c("18h", "20h")),
      aes(fill = value),
    ...
    scale_fill_gradientn(
      colours = c("white","#66C2BE"),limits = c(vmin, vmax),
      breaks = seq(vmin, vmax, length.out = 2), name = "18h and 20h") +
    # Text overlay
    ...
    scale_y_discrete(expand = c(0, 0)) +
    labs(
      title = "Setup distribution across individuals",
      x = "Setup",
      y = "Individual"
    )
  ...
  # Save data to plot
  write.table(tabm, file.path(data_to_plot_dir, "004_Setup_distribution_across_individuals.txt"),
              sep = "\t", quote = FALSE, row.names = TRUE)
  write.table(tab_ind, file.path(data_to_plot_dir, "004_Setup_distribution_across_individuals_summary.txt"),
              sep = "\t", quote = FALSE, row.names = TRUE)
  
  
  pdf(file.path(QC_dir,"003_Setup_distribution_across_flowcells.pdf"),width=12,height=6)
  print(pl_tiles)
  dev.off()
  
  pdf(file.path(QC_dir,"004_Setup_distribution_across_individuals.pdf"),width=8,height=16)
  print(pl_tot)
  dev.off()
...
##############
{
  ...
  # reads_whole = reads_whole[, cols$Sample]
...
##############
{
  ...
  get_PCA=function(setups,name,type="infection"){
    ...
    pl=ggplot(datos) +
      geom_point(data=datos,aes(x=PC1, y=PC2,colour=Setup),size=3,alpha=0.9)+
  ...
  ###########################################################################
  ##  10. Upon visual identification of mislabels, I correct the metadata  ##
  ###########################################################################
...
##############
{
  ...
  ##  bit less aggressively (or I would be trashing too many 20h samples,  ##
  ...
  ##        , or still go with|n the other cluster in the tsne (I_C13_TB_18h).       ##
  #####################################################################################
  ...
  ##                   "I_C5_TB_72h","I_C14_NI_72h","I_C13_TB_20h" ) ##
  #####################################################################
...
##############
{
  ...
  DE_before_dir <- file.path(input_dir,"002_Processed","DE_tables_before_remove_samples")
  dir.create(DE_before_dir,showWarnings = F,recursive = T)
  write.table(fdrs,file.path(DE_before_dir,"FDRs_raw.txt"))
  
  contrast_fitter=function(contrast,dir,contrast_name){
  ...
  ### Fit contrasts
  res_contrast_2_to_18_NI=contrast_fitter(contrast_2_to_18_NI,DE_before_dir,"contrast_2_to_18_NI")
  #6291
  res_contrast_2_to_20_NI=contrast_fitter(contrast_2_to_20_NI,DE_before_dir,"contrast_2_to_20_NI")
  #3570
  res_contrast_2_to_48_NI=contrast_fitter(contrast_2_to_48_NI,DE_before_dir,"contrast_2_to_48_NI")
  #7541
  res_contrast_2_to_72_NI=contrast_fitter(contrast_2_to_72_NI,DE_before_dir,"contrast_2_to_72_NI")
  #7736
  res_contrast_18_to_20_NI=contrast_fitter(contrast_18_to_20_NI,DE_before_dir,"contrast_18_to_20_NI")
  #53
  res_contrast_18_to_48_NI=contrast_fitter(contrast_18_to_48_NI,DE_before_dir,"contrast_18_to_48_NI")
  #2719
  res_contrast_18_to_72_NI=contrast_fitter(contrast_18_to_72_NI,DE_before_dir,"contrast_18_to_72_NI")
  #4179
  res_contrast_20_to_48_NI=contrast_fitter(contrast_20_to_48_NI,DE_before_dir,"contrast_20_to_48_NI")
  #1022
  res_contrast_20_to_72_NI=contrast_fitter(contrast_20_to_72_NI,DE_before_dir,"contrast_20_to_72_NI")
  #1942
  res_contrast_48_to_72_NI=contrast_fitter(contrast_48_to_72_NI,DE_before_dir,"contrast_48_to_72_NI")
  #463
  res_contrast_2_to_18_TB=contrast_fitter(contrast_2_to_18_TB,DE_before_dir,"contrast_2_to_18_TB")
  #6770
  res_contrast_2_to_20_TB=contrast_fitter(contrast_2_to_20_TB,DE_before_dir,"contrast_2_to_20_TB")
  #3943
  res_contrast_2_to_48_TB=contrast_fitter(contrast_2_to_48_TB,DE_before_dir,"contrast_2_to_48_TB")
  #7629
  res_contrast_2_to_72_TB=contrast_fitter(contrast_2_to_72_TB,DE_before_dir,"contrast_2_to_72_TB")
  #8120
  res_contrast_18_to_20_TB=contrast_fitter(contrast_18_to_20_TB,DE_before_dir,"contrast_18_to_20_TB")
  #844
  res_contrast_18_to_48_TB=contrast_fitter(contrast_18_to_48_TB,DE_before_dir,"contrast_18_to_48_TB")
  #2151
  res_contrast_18_to_72_TB=contrast_fitter(contrast_18_to_72_TB,DE_before_dir,"contrast_18_to_72_TB")
  #4073
  res_contrast_20_to_48_TB=contrast_fitter(contrast_20_to_48_TB,DE_before_dir,"contrast_20_to_48_TB")
  #1567
  res_contrast_20_to_72_TB=contrast_fitter(contrast_20_to_72_TB,DE_before_dir,"contrast_20_to_72_TB")
  #3518
  res_contrast_48_to_72_TB=contrast_fitter(contrast_48_to_72_TB,DE_before_dir,"contrast_48_to_72_TB")
  #781
  
  res_contrast_2_to_18_TB_int=contrast_fitter(contrast_2_to_18_TB_int,DE_before_dir,"contrast_2_to_18_TB_int")
  #[1] 3472 3810
  res_contrast_2_to_20_TB_int=contrast_fitter(contrast_2_to_20_TB_int,DE_before_dir,"contrast_2_to_20_TB_int")
  #[1] 621 590
  res_contrast_2_to_48_TB_int=contrast_fitter(contrast_2_to_48_TB_int,DE_before_dir,"contrast_2_to_48_TB_int")
  #[1] 5062 5134
  res_contrast_2_to_72_TB_int=contrast_fitter(contrast_2_to_72_TB_int,DE_before_dir,"contrast_2_to_72_TB_int")
  #[1] 5809 5982
  res_contrast_18_to_20_TB_int=contrast_fitter(contrast_18_to_20_TB_int,DE_before_dir,"contrast_18_to_20_TB_int")
  #[1] 452 389
  res_contrast_18_to_48_TB_int=contrast_fitter(contrast_18_to_48_TB_int,DE_before_dir,"contrast_18_to_48_TB_int")
  #[1] 825 820
  res_contrast_18_to_72_TB_int=contrast_fitter(contrast_18_to_72_TB_int,DE_before_dir,"contrast_18_to_72_TB_int")
  #[1] 1658 1784
  res_contrast_20_to_48_TB_int=contrast_fitter(contrast_20_to_48_TB_int,DE_before_dir,"contrast_20_to_48_TB_int")
  #[1] 373 319
  res_contrast_20_to_72_TB_int=contrast_fitter(contrast_20_to_72_TB_int,DE_before_dir,"contrast_20_to_72_TB_int")
  #[1] 1184 1202
  res_contrast_48_to_72_TB_int=contrast_fitter(contrast_48_to_72_TB_int,DE_before_dir,"contrast_48_to_72_TB_int")
  #[1] 34 49
  ...
  DE_after_dir <- file.path(input_dir,"002_Processed","DE_tables_after_remove_samples")
  dir.create(DE_after_dir,showWarnings = F,recursive = T)
  write.table(fdrs,file.path(DE_after_dir,"FDRs_after_rm_outliers.txt.txt"))
  ...
  res_contrast_2_to_18_NI=contrast_fitter(contrast_2_to_18_NI,DE_after_dir,"contrast_2_to_18_NI")
  #6291 6443
  res_contrast_2_to_20_NI=contrast_fitter(contrast_2_to_20_NI,DE_after_dir,"contrast_2_to_20_NI")
  #3570 3684
  res_contrast_2_to_48_NI=contrast_fitter(contrast_2_to_48_NI,DE_after_dir,"contrast_2_to_48_NI")
  #7541 7594
  res_contrast_2_to_72_NI=contrast_fitter(contrast_2_to_72_NI,DE_after_dir,"contrast_2_to_72_NI")
  #7736 7863
  res_contrast_18_to_20_NI=contrast_fitter(contrast_18_to_20_NI,DE_after_dir,"contrast_18_to_20_NI")
  #53 62
  res_contrast_18_to_48_NI=contrast_fitter(contrast_18_to_48_NI,DE_after_dir,"contrast_18_to_48_NI")
  #2719 2775
  res_contrast_18_to_72_NI=contrast_fitter(contrast_18_to_72_NI,DE_after_dir,"contrast_18_to_72_NI")
  #4179 4312
  res_contrast_20_to_48_NI=contrast_fitter(contrast_20_to_48_NI,DE_after_dir,"contrast_20_to_48_NI")
  #1022 1057
  res_contrast_20_to_72_NI=contrast_fitter(contrast_20_to_72_NI,DE_after_dir,"contrast_20_to_72_NI")
  #1942 2124
  res_contrast_48_to_72_NI=contrast_fitter(contrast_48_to_72_NI,DE_after_dir,"contrast_48_to_72_NI")
  #463 499
  res_contrast_2_to_18_TB=contrast_fitter(contrast_2_to_18_TB,DE_after_dir,"contrast_2_to_18_TB")
  #6770 6930
  res_contrast_2_to_20_TB=contrast_fitter(contrast_2_to_20_TB,DE_after_dir,"contrast_2_to_20_TB")
  #3943 3888
  res_contrast_2_to_48_TB=contrast_fitter(contrast_2_to_48_TB,DE_after_dir,"contrast_2_to_48_TB")
  #7629 7705
  res_contrast_2_to_72_TB=contrast_fitter(contrast_2_to_72_TB,DE_after_dir,"contrast_2_to_72_TB")
  #8120 8159
  res_contrast_18_to_20_TB=contrast_fitter(contrast_18_to_20_TB,DE_after_dir,"contrast_18_to_20_TB")
  #844 635
  res_contrast_18_to_48_TB=contrast_fitter(contrast_18_to_48_TB,DE_after_dir,"contrast_18_to_48_TB")
  #2151 2100
  res_contrast_18_to_72_TB=contrast_fitter(contrast_18_to_72_TB,DE_after_dir,"contrast_18_to_72_TB")
  #4073 4081
  res_contrast_20_to_48_TB=contrast_fitter(contrast_20_to_48_TB,DE_after_dir,"contrast_20_to_48_TB")
  #1567 1369
  res_contrast_20_to_72_TB=contrast_fitter(contrast_20_to_72_TB,DE_after_dir,"contrast_20_to_72_TB")
  #3518 3329
  res_contrast_48_to_72_TB=contrast_fitter(contrast_48_to_72_TB,DE_after_dir,"contrast_48_to_72_TB")
  #781 806
  
  res_contrast_2_to_18_TB_int=contrast_fitter(contrast_2_to_18_TB_int,DE_after_dir,"contrast_2_to_18_TB_int")
  #[1] 3472 3913
  res_contrast_2_to_20_TB_int=contrast_fitter(contrast_2_to_20_TB_int,DE_after_dir,"contrast_2_to_20_TB_int")
  #[1] 621 599
  res_contrast_2_to_48_TB_int=contrast_fitter(contrast_2_to_48_TB_int,DE_after_dir,"contrast_2_to_48_TB_int")
  #[1] 5062 5129
  res_contrast_2_to_72_TB_int=contrast_fitter(contrast_2_to_72_TB_int,DE_after_dir,"contrast_2_to_72_TB_int")
  #[1] 5809 6014
  res_contrast_18_to_20_TB_int=contrast_fitter(contrast_18_to_20_TB_int,DE_after_dir,"contrast_18_to_20_TB_int")
  #[1] 452 418
  res_contrast_18_to_48_TB_int=contrast_fitter(contrast_18_to_48_TB_int,DE_after_dir,"contrast_18_to_48_TB_int")
  #[1] 825 788
  res_contrast_18_to_72_TB_int=contrast_fitter(contrast_18_to_72_TB_int,DE_after_dir,"contrast_18_to_72_TB_int")
  #[1] 1658 1763
  res_contrast_20_to_48_TB_int=contrast_fitter(contrast_20_to_48_TB_int,DE_after_dir,"contrast_20_to_48_TB_int")
  #[1] 373 323
  res_contrast_20_to_72_TB_int=contrast_fitter(contrast_20_to_72_TB_int,DE_after_dir,"contrast_20_to_72_TB_int")
  #[1] 1184 1260
  res_contrast_48_to_72_TB_int=contrast_fitter(contrast_48_to_72_TB_int,DE_after_dir,"contrast_48_to_72_TB_int")
  #[1] 34 63
...
##############
{
  ...
  dim(reads)[1]
  # 11494
  # We have 63 id without hugo names.???
  
  hugos=hugos[which(hugos$gene_biotype=="protein_coding"),]
  dim(hugos)[1]
  # 10753 protein coding genes with hugo names.
  
  ### Check for duplicates by ensemble id
  length(which(duplicated(hugos$ensembl_gene_id)))
  ### Check for duplicates by hgnc symbol
  duplicated_hugo_id <- hugos[which(duplicated(hugos$hgnc_symbol)),]
  # We have 48 entrez ids without hugo names.
  ...
  ## 49 non-available gene names, I give them the ensembl ID also in the 
  ...
  length(which(hugos$hgnc_symbol!=check_genes$Suggested.Symbol))
  ...
  dictionary <- dictionary[order(rownames(reads)),]
...
##############
{
  ...
  ### Here I see that there are some individual IDs in RNA that are not in DNA and viceversa
  ...
  ### That way I correct the sample IDs in the DNA data to match the RNA ones by hand
  ...
  ### La escribo en un xlsx file para editar a mano los errores
  library(openxlsx)
...
##############
{
  ...
  do_tsne=function(genes_set=0,input="clean",perp=30,iters=1000,genes=1000,seed){
    ...
    pl=ggplot(tab)+
      geom_point(aes(x=tsne_1, y=tsne_2,colour=Setup),size=3,alpha=0.9)+
      #geom_text_repel(aes(x=PC1, y=PC2,label=label))+
      guides(color = guide_legend(override.aes = list(size=3)))+
      scale_colour_manual(values=colores)+
      xlab("t-SNE 1")+
  ...
  pdf(file.path(tsne_dir,"tsne.pdf"),width=10,height=10)
  print(tsne_clean[[1]])
  dev.off()
