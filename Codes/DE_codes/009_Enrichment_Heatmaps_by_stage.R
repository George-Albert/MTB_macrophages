...
#################################
{
  ...
  script_id          <- "009_Enrichment_Heatmaps_by_stage"
  ...
  enrichment_dir     <- file.path(output_dir_prev,"005_Enrichment_GO")
  figures_dir        <- file.path(output_dir,"001_Figures")
  data_to_plot_dir   <- file.path(output_dir,"002_Data_to_plot")
  
  dir.create(output_dir,showWarnings = F)
  dir.create(figures_dir,showWarnings = F)
  dir.create(data_to_plot_dir,showWarnings = F)
  
  # config_dir       <- "3rd_Configuration_by_stage"
  # config_dir       <- "1st_Configuration_by_stage"
  # config_dir       <- "2nd_Configuration_by_stage"
  # config_dir       <- "4th_Configuration_by_stage"
  dir_of_gene_list <- list.files(enrichment_dir,pattern = "Configuration")
...
##############
{
  ...
  for (j in seq_along(dir_of_gene_list)) {
    
    dir <- file.path(enrichment_dir,dir_of_gene_list[j])
    all_file_name <- list.files(dir,pattern = ".txt",recursive = T)
    file_name <- all_file_name[!grepl("community", all_file_name) & 
                                 !grepl("Configuration", all_file_name)]
    list_enrichment <- lapply(file.path(dir,file_name), read.table)
    names(list_enrichment) <- gsub("/.*","",file_name)
    
    # Define the lists to loop over up and down
    list_enrichment_up   <- list_enrichment[c(2,4,6)]
    ...
    # list_name_to_loop       <- c("List_UP_wo_clustering","List_Down_wo_clustering")
    
    # level_index             <- data.frame(UP=c("Early.UP","Mid.UP","Late.Up","Persistent.UP"),
    #                                      Down= c("Early.Down","Mid.Down","Late.Down","Persistent.Down"))
    level_index             <- data.frame(UP=c("Early.Up","Mid.Up","Late.Up"),
                                          Down= c("Early.Down","Mid.Down","Late.Down"))
    
    for (i in seq_along(list_enrichment_to_loop)) {
      
      # Select the list to analyze
      list_enrichment <- list_enrichment_to_loop[[i]][1:3]
      # Define the name of the list to analyze
      ...
      list_enrichment_simplyfied <- lapply(names(list_enrichment), function(name) {
        ...
        df <- list_enrichment[[name]]  # Extract the data frame
        df <- df[c("GO_ID", "description", "levels", "OR", "p", "fdr", "Min_value_CI", 
                   "Max_value_CI", "genes_in_query")]  # Select desired columns
        df$Cluster <-  name  # Add column with the name of the cluster
        return(df)
      })
      ...
      # th_qB <- 0.2  # quantile for FDR
      # th_qOR <- 0.95 # quantile for OR
      # th_qp <- 0.05  # quantile for p value
      filter_list <- function(x, th_qB=0.2, th_qOR=0.95,th_qp=0.05) {
        ...
        # th_p <- quantile(x$fdr,th_qB)
        print(th_p)
        # Establish the hit column
        x$hit=0
        x$hit[which(x$OR > th_OR | x$p < th_p)] = 1
        
        # print(paste0(names(x)," ", length(which(x$hit==1))," th_OR=",th_OR," th_p=",th_p))
        # Order the data frame by OR and p value
        x=x[order(-x$OR),]
        # Compute the rank of the OR
        x$Rank_OR=c(1:nrow(x))
        # Compute the rank of the p value
        x=x[order(x$p),]
        # x=x[order(x$fdr),]
        x$Rank_p=c(1:nrow(x))
        # Compute the rank of the sum of the ranks euclidean distance
      ...
      color_breaks <- seq(-2, 3.5, length.out = 11)  # Adjust according to data
      ...
      write.table(hclust_df,file = file.path(data_to_plot_dir,paste0("Cluster_by_stage_sortby_OR_p_value_",list_name,".txt")))
      
      dir.create(file.path(output_dir,"001_Figures","005_Enrichment",dir_of_gene_list[j]),recursive=T,showWarnings =F)
      filename <- file.path(output_dir,"001_Figures","005_Enrichment",dir_of_gene_list[j],paste0("Heatmap_GO_by_stage_",list_name,".pdf"))
